...
source_aarch64 = ["#{pkgname.sub(/-bin$/, '')}-#{pkgver}-aarch64.rpm::#{_ghurl}/releases/download/v#{pkgver}/#{_pkgname}-v#{pkgver}-linux-arm64.rpm"]
source_x86_64 = ["#{pkgname.sub(/-bin$/, '')}-#{pkgver}-x86_64.rpm::#{_ghurl}/releases/download/v#{pkgver}/#{_pkgname}-v#{pkgver}-linux-amd64.rpm"]
...
def prepare(pkgname, _pkgname, srcdir)
    system("sed", "-i", "-e", "
...
def package(pkgname, srcdir, pkgdir)
    ...
    system("cp", "-Pr", "--no-preserve=ownership", "#{srcdir}/opt/#{pkgname.sub(/-bin$/, '')}", "#{pkgdir}/usr/lib")
